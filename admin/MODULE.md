...
| Knowledge (Skills & Training) | Combined tabbed page (`Knowledge.svelte`) with two tabs: **Skills** (`Skills.svelte`) for skill management, registry browser, import/export, search, playbook content preview, and related skills navigation. Skill create modal includes name, slug, program, category (with `project-reference` and `housekeeping` options), description, content, priority, autoFetch, and enabled fields. The skill **edit panel now includes a Related Skills picker** (combobox dropdown of existing slugs + removable chips for currently-linked skills; submits via `relatedSkills` in the `PUT /api/skills/:slug` body) so admins can build the knowledge graph without hand-editing JSON. The skill list **filter bar matches the desktop client** — Program, Type (API vs DCC, driven by the loaded `/api/api-bridges` list), Category, Source, Status (enabled/disabled), Keywords, free-text search, plus an active-filter count badge and "Clear" button. The lock toggle now actually persists (previously the `locked` field was silently dropped from `SkillUpdateSchema`, so the admin toggle looked like it worked but vanished on reload). **Training Vault** (`CoordinatorTraining.svelte`) for vault explorer, repository controls, and snapshots. Single sidebar entry "Skills & Training" replaces the former separate "Skills" and "Training Vault" entries. |
...
| Templates | Template management page with sub-tabs for Chat Prompts, Project Prompts, Job Presets, and Path Mappings (`type` filter: `chat`, `project`, `job_preset`, `path_mapping`). Category-grouped table with name, description, subcategory, sort order, enabled toggle, and edit/delete actions. Create/edit modal with fields for name, type, category, subcategory, description, content (textarea), icon, sort order, enabled toggle, plus job_preset-specific fields (verificationMode, verificationWeight, bridgeExecutionMode), plus path_mapping-specific platform+path entry list editor (stored as `options.entries`). "Seed Defaults" button to populate built-in templates via `POST /api/templates/seed`. |
...
| System | System administration page with a tab bar: **System Settings** tab (job timeout, max concurrent agents, log level, worker poll interval, default workspace mode) backed by `GET/PUT /api/settings/system-config`, and **Danger Zone** tab containing Factory Reset functionality. **Community Skills (arkestrator.com) panel (added):** the System Settings tab now embeds a full prompt-injection-defense control surface for community skills. Top section: an effective-state banner that combines all the policy flags into one of four labels (`HARD-DISABLED` / `DISABLED` / `ENABLED — relaxed framing` / `ENABLED — extra caution on`) plus a counts strip (total installed, flagged, by trust tier — `verified`/`community`/etc.) and a `Delete all N flagged` bulk-action button when flagged > 0. Middle section: four security toggles in danger-tinted cards — **Hard-disable community skills** (master kill switch, locks the per-user toggle in clients, recommended for shared servers), **Allow community skills on this server** (default for the per-user toggle), **Extra caution mode** (aggressive untrusted-content framing in agent prompts, recommended on), and the legacy **Enable agent auto-install** toggle. All four are wired to `setCommunityAdminHardDisabled` / `setCommunityAllowOnClient` / `setCommunityExtraCaution` / `setCommunityAgentAutoInstall` in the api facade and disabled when hard-disable is on. Bottom section: an **Installed Community Skills** table listing slug, title, trust tier (color-coded `verified`/`community`/`unknown`), author (`@login` + verified tick), per-skill flagged status with hovered reasons (reasons are now humanized via a local `RULE_LABELS` map so marketplace-side names like `ignore_previous` and local-scanner names like `ignore_previous_instructions` collapse to a single shared label like "Ignore previous instructions" — the raw pattern names are still visible in the cell's `title` tooltip for debugging), and a per-row delete button calling `deleteCommunitySkill(slug, program)`. Flagged rows are highlighted; the table caps at 20 rows by default with a "Show all" expander. Backed by new endpoints `GET /api/settings/community/stats`, `DELETE /api/settings/community/skills/flagged`, `DELETE /api/settings/community/skills/:slug`. **Danger Zone** tab contains Factory Reset functionality. Factory Reset requires password + typing "RESET" to confirm. |
...
- `settings.*` community methods: `getCommunityAdmin` (now also returns `adminHardDisabled`/`allowOnClient`/`extraCaution`), `setCommunityAgentAutoInstall`, `setCommunityBaseUrl`, `setCommunityAdminHardDisabled` (master kill switch), `setCommunityAllowOnClient` (per-server default for the user toggle), `setCommunityExtraCaution` (untrusted-content framing aggressiveness), `clearCommunityUserSession`, `getCommunityStats` (counts + flagged-skills list for the admin panel), `deleteCommunitySkill(slug, program)` (single delete, refuses non-community sources), `deleteAllFlaggedCommunitySkills` (bulk delete every flagged community skill).
