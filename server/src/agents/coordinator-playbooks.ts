  ...
  global: {
    ...
    files: {
      "tasks/cross_bridge_workflow.md": [
        ...
        "3. For any visual output (renders, textures, images, flipbooks): visually verify using read_client_file to load the file, then Read to inspect it. Confirm content matches the request, quality is acceptable, and there are no artifacts or errors. Do NOT pass bad output to downstream stages.",
        "4. Keep naming/versioning consistent across generated files.",
      ].join("\n"),
  ...
  blender: {
    ...
    files: {
      "tasks/scene_build_render.md": [
        ...
        "4. ALWAYS visually verify renders: use read_client_file to load the output, then Read to inspect it. Check that:",
        "   - The render matches what was requested (correct subject, composition, lighting)",
        "   - Materials and textures look correct (no missing textures, wrong colors, or placeholder materials)",
        "   - If a reference image was provided, compare visually against it",
        "   - No render artifacts (fireflies, black areas, missing geometry)",
        "5. Do NOT mark a render as successful if the output is clearly wrong — fix and re-render.",
      ].join("\n"),
    },
  },
  godot: {
  ...
  houdini: {
    ...
    files: {
      "tasks/general_houdini_workflow.md": [
        ...
        "5. For render/flipbook outputs: visually verify using read_client_file + Read. Check content matches the request, materials/shading are correct, and no render artifacts are present.",
        "6. Do NOT mark renders as successful if the output is clearly wrong — fix and re-render.",
      ].join("\n"),
    },
  },
  unity: {
    ...
    files: {
      "tasks/scene_prefab_setup.md": [
        ...
        "4. For visual outputs (screenshots, renders): visually verify using read_client_file + Read to ensure content matches the request.",
      ].join("\n"),
    },
  },
  unreal: {
    ...
    files: {
      "tasks/level_blueprint_flow.md": [
        ...
        "4. For visual outputs (renders, screenshots): visually verify using read_client_file + Read to ensure content matches the request.",
      ].join("\n"),
    },
  },
  comfyui: {
    ...
    files: {
      "tasks/workflow_generation.md": [
        ...
        "4. ALWAYS visually verify generated images/textures using read_client_file to load them, then Read to inspect. Check for:",
        "   - Correct content matching the prompt (not garbled, wrong subject, or artifacts)",
        "   - Clean quality (no obvious noise, banding, or generation failures)",
        "   - Tileable/seamless if requested (no visible seams or cross-shaped blend artifacts)",
        "   - Correct resolution and aspect ratio",
        "5. If the output looks wrong, re-queue with adjusted parameters — do NOT mark bad output as successful.",
      ].join("\n"),
    },
  },
};
